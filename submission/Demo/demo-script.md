...
demo day; the examiner asks questions _during_ the demo, so know the material well
...
2. Open **Requests**, click an existing request, show the history timeline:
   _"every state change is logged with who, when and why."_
...
the seed command _and_ the IoT rules — there is no side door.
...
5. Post another high reading: _no duplicate ticket_ — explain dedupe keys.
